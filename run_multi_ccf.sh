...
## Number of time bins to plot along the 2D CCF x-axis
tlen=50
obs_epoch=5
...
if (( $testing == 0 )); then
	...
	saved_file_list="$ccf_out_dir/${filename}_filelist"
	...
	saved_file_list="$ccf_out_dir/test_${filename}_filelist"
...
#	python "$ccf_exe_dir"/multi_ccf.py "$saved_file_list" "${ccf_out_file}" \
...
#		-p "$prefix" --ext "$p_ext"
#	if [ -e "${ccf_plot_root}_chan_15.${p_ext}" ]; then
#       open "${ccf_plot_root}_chan_15.${p_ext}"; fi
#
#	ccf_multi_plot="${ccf_plot_root}_multiccfs.${p_ext}"
...
########################
#
#ccf_plot_file="${ccf_plot_root}_2Dccf.${p_ext}"
#if [ -e "${out_file}" ]; then
#	python "$ccf_exe_dir"/plot_2d.py "${ccf_out_file}" -o "${ccf_plot_file}" \
#		-p "$prefix" -l "$tlen" -e "$energies_file"
#	if [ -e "${ccf_plot_file}" ]; then
#		open "${ccf_plot_file}"
##        cp "$ccf_plot_file" "$home_dir/Dropbox/Research/CCF_paper1/"
#    fi
#fi
#
#detchans=$(python -c "import tools; print int(tools.get_key_val('${ccf_out_file}', 0, 'DETCHANS'))")
#tlen2=$(( 2*tlen ))
#
#if [ -e "$ccf_exe_dir/temp.dat" ]; then
#	fimgcreate bitpix=-32 \
#		naxes="${tlen2},${detchans}" \
#		datafile="$exe_dir/temp.dat" \
#		outfile="${ccf_plot_root}_2Dccf.fits" \
#		nskip=1 \
#		history=true \
#		clobber=yes
#else
...
#fi
#
#if [ -e "${ccf_plot_root}_2Dccf.fits" ]; then
#	echo "FITS 2D ccf ratio image: ${ccf_plot_root}_2Dccf.fits"
#else
...
#	fi
	...
	echo -e "\tERROR: get_lags.py was not run. Lag output file does not exist."
fi
