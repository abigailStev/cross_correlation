...
#########################################################################################
def filter_freq_noise(cs_avg, dt, n_bins):
	"""
			filter_freq_noise
	...
	Passed: cs_avg - The average cross spectrum over all segments (and all files, 
				if applicable).
			dt - Time step between bins, in seconds. Must be a power of 2.
	
	Returns: smoothed_cs_avg_i
	
	"""
	
# 	print "Length of cross spectrum before filter =", len(cs_avg_i)
...
# 	print j_min, j_max
# 	print "Shape of cs avg", np.shape(cs_avg)
	smooth_front = np.zeros(j_min)
# 	print "Length of cs avg", len(cs_avg)
	smooth_end = np.zeros((len(cs_avg) - j_max))
# 	print len(smooth_front)+len(cs_avg[j_min:j_max+1])+len(smooth_end)
# 	smoothed_cs_avg = np.zeros((n_bins,64), dtype=np.complex128)
	...
	smoothed_cs_avg = np.concatenate((smooth_front_alt, cs_avg[j_min:j_max,:],
                                         smooth_end_alt))
# 	print "Length of cross spectrum after filter =", len(smoothed_cs_avg_i)
# 	print "Shape of smoothed cs avg", np.shape(smoothed_cs_avg)
	return smoothed_cs_avg
...
# 	print "bins =", n_bins
	...
	cs_avg = np.sqrt(cs_avg * (2.0 * dt / float(n_bins)) - (2.0 * mean_rate_whole_ref))
