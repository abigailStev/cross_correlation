...
def fits_out(out_file, in_file, bkgd_file, dt, n_bins, detchans, num_seconds,
             num_seg, mean_rate_ci_whole, mean_rate_ref_whole, t, ccf,
             ccf_error,
             filter):
    """
			fits_out
...
def make_lags(out_file, in_file, dt, n_bins, detchans, num_seconds,
    ...
    phase = -np.arctan2(cs_avg.imag,
                        cs_avg.real)  ## Negative sign is so that a positive lag is a hard energy lag?
    ...
    e_phase = -np.arctan2(frange_cs.imag,
                          frange_cs.real)  ## Negative sign is so that a positive lag is a hard energy lag?
...
def save_for_lags(out_file, in_file, dt, n_bins, detchans, num_seconds, \
                  num_seg, mean_rate_ci, mean_rate_ref, cs_avg, power_ci,
                  power_ref):
    """
    ...
    prihdr = fits.Header()
    prihdr.set('TYPE',
               "Cross spectrum, power spectrum ci, and power spectrum ref.")
    prihdr.set('DATE', str(datetime.now()), "YYYY-MM-DD localtime")
    prihdr.set('EVTLIST', in_file)
    prihdr.set('DT', dt, "seconds")
    prihdr.set('N_BINS', n_bins, "time bins per segment")
...
def FILT_cs_to_ccf_w_err(cs_avg, dt, n_bins, detchans, num_seconds,
                         total_segments, countrate_ci, countrate_ref, power_ci,
                         power_ref, noisy):
    """
    ...
    with np.errstate(all='ignore'):
        error_ratio = np.where(cs_signal_amp != 0, cs_noise_amp / cs_signal_amp,
                               0)
...
def standard_ccf_err(n_bins, detchans, num_seg):
    ...
    for i in range(detchans):
        ...
        sample_var_i = np.sum(ccf_resid_i ** 2, axis=0) / float(
            num_seg - 1)  ## eq 2.3
...
def UNFILT_cs_to_ccf_w_err(cs_avg, dt, n_bins, detchans, num_seconds,
                           num_seg, countrate_ci, countrate_ref, power_ci,
                           power_ref, noisy):
    """
...
def each_segment(time_ci, time_ref, energy_ci, energy_ref, n_bins, detchans,
                 dt, start_time, end_time, obs_epoch, sum_rate_ci_whole,
                 sum_rate_ref_whole,
    ...
    while seg_end_time < final_time:
        ...
        if len(time_ci) > 0 and len(time_ref) > 0:
            ...
            sum_power_ref, sum_rate_ci = each_segment(time_ci, time_ref,
                                                      energy_ci, energy_ref,
                                                      n_bins, detchans, dt,
                                                      start_time,
                                                      seg_end_time, obs_epoch,
                                                      sum_rate_ci_whole,
                                                      sum_rate_ref_whole,
                                                      sum_power_ci,
                                                      sum_power_ref, cs_sum,
                                                      sum_rate_ci)
                    ...
                    if len(time_ci) > 0 and len(time_ref) > 0:
                        ...
                        sum_power_ci, sum_power_ref, sum_rate_ci = \
                            each_segment(time_ci, time_ref, energy_ci,
                                         energy_ref, n_bins, detchans, dt,
                                         start_time,
                                         end_time, obs_epoch, sum_rate_ci_whole,
                                         sum_rate_ref_whole, sum_power_ci,
                                         sum_power_ref,
                                         cs_sum, sum_rate_ci)
...
def read_and_use_segments(in_file, n_bins, detchans, dt, test):
    ...
    if (in_file[-5:].lower() == ".fits"):
        ...
        fits_file = in_file[
                    0:-4] + ".fits"  ## Still need a fits file to get the observation time
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(usage="python ccf.py infile outfile [-b " \
                                     ...
                                     description="Computes the cross-correlation function of a channel of " \
                                                 "interest with a reference band.",
                                     epilog="For optional arguments, " \
